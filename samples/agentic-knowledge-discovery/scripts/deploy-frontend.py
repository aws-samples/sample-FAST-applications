...
def parse_config_yaml(config_path: Path) -> Dict[str, str]:
    ...
    """
    config = {"stack_name_base": "", "pattern": "strands-agent"}
...
def generate_aws_exports(
    ...
    aws_exports = {
        ...
        # Session history shares the same API Gateway base URL as feedback.
        "sessionsApiUrl": outputs["FeedbackApiUrl"],
...
def main() -> int:
    ...
    config = parse_config_yaml(config_path)
    pattern = config.get("pattern", "strands-agent")
