...
APPROACH 1 (Active): Direct Cognito token call with user identity propagation.
  ...
  role) for an authorization layer at the Gateway (an extension point; not enforced in this sample). The user_id from the validated
...
def create_gateway_mcp_client(user_id: str) -> MCPClient:
    ...
    as an extension point for Gateway-side authorization (not enforced in this sample).
