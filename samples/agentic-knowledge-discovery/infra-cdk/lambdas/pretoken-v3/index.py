...
ALTERNATIVE (email-based matching without two-step deploy):
  ...
  UUID lookup, add email resolution via the Cognito ListUsers API in this
  Lambda and match on email instead of sub.

To use dynamic group assignment, replace the hardcoded mapping below with a
DynamoDB table keyed by the user's sub (UUID).
"""
...
def lambda_handler(event: dict, context: dict) -> dict:
    ...
    # above with your actual Cognito sub.
