...
export class BackendStack extends cdk.NestedStack {
  ...
  private reservationsTable: dynamodb.Table
  ...
  constructor(scope: Construct, id: string, props: BackendStackProps) {
    ...
    this.reservationsTable = this.createReservationsTable(props.config)
  ...
  private createAgentCoreRuntime(config: AppConfig): void {
    ...
    // Add Knowledge Base retrieve permissions
    agentRole.addToPolicy(
      new iam.PolicyStatement({
    ...
    const envVars: { [key: string]: string } = {
      ...
      GATEWAY_CREDENTIAL_PROVIDER_NAME: `${config.stack_name_base}-runtime-gateway-auth`,
      KNOWLEDGE_BASE_ID: cdk.Fn.importValue(`${config.stack_name_base}-kb-KnowledgeBaseId`),
    }
  ...
  /**
   * Creates a DynamoDB table for storing restaurant reservations.
   *
   * @param config - Application configuration.
   * @returns The DynamoDB table resource.
   */
  private createReservationsTable(config: AppConfig): dynamodb.Table {
    const table = new dynamodb.Table(this, "ReservationsTable", {
      tableName: `${config.stack_name_base}-reservations`,
      partitionKey: { name: "booking_id", type: dynamodb.AttributeType.STRING },
      sortKey: { name: "restaurant_name", type: dynamodb.AttributeType.STRING },
      billingMode: dynamodb.BillingMode.PAY_PER_REQUEST,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
      encryption: dynamodb.TableEncryption.AWS_MANAGED,
    })

    new ssm.StringParameter(this, "ReservationsTableNameParam", {
      parameterName: `/${config.stack_name_base}/reservations-table-name`,
      stringValue: table.tableName,
      description: "DynamoDB table name for restaurant reservations",
    })
  ...
  private createFeedbackApi(
    ...
    // Create presigned URL Lambda for KB file uploads
    const presignedUrlLambda = new PythonFunction(this, "PresignedUrlLambda", {
      ...
      handler: "handler",
      environment: {
        KB_BUCKET_NAME: cdk.Fn.importValue(`${config.stack_name_base}-kb-DataBucketName`),
      },
      timeout: cdk.Duration.seconds(30),
      layers: [
        lambda.LayerVersion.fromLayerVersionArn(
          this,
          "PresignedUrlPowertoolsLayer",
          `arn:aws:lambda:${
            cdk.Stack.of(this).region
          }:017000801446:layer:AWSLambdaPowertoolsPythonV3-python313-arm64:18`
        ),
    ...
    // Grant S3 put permissions for presigned URL generation
    presignedUrlLambda.addToRolePolicy(
      new iam.PolicyStatement({
        effect: iam.Effect.ALLOW,
        actions: ["s3:PutObject"],
        resources: [
          `arn:aws:s3:::${cdk.Fn.importValue(`${config.stack_name_base}-kb-DataBucketName`)}/*`,
        ],
      })
    )

    // Create sync KB Lambda
    const syncKbLambda = new PythonFunction(this, "SyncKbLambda", {
      functionName: `${config.stack_name_base}-sync-kb`,
      runtime: lambda.Runtime.PYTHON_3_13,
      architecture: lambda.Architecture.ARM_64,
      entry: path.join(__dirname, "..", "lambdas", "sync-kb"),
      handler: "handler",
      environment: {
        KNOWLEDGE_BASE_ID: cdk.Fn.importValue(`${config.stack_name_base}-kb-KnowledgeBaseId`),
        DATA_SOURCE_ID: cdk.Fn.importValue(`${config.stack_name_base}-kb-DataSourceId`),
      },
      timeout: cdk.Duration.seconds(30),
      layers: [
        lambda.LayerVersion.fromLayerVersionArn(
          this,
          "SyncKbPowertoolsLayer",
          `arn:aws:lambda:${
            cdk.Stack.of(this).region
          }:017000801446:layer:AWSLambdaPowertoolsPythonV3-python313-arm64:18`
        ),
      ],
      logGroup: new logs.LogGroup(this, "SyncKbLambdaLogGroup", {
        logGroupName: `/aws/lambda/${config.stack_name_base}-sync-kb`,
        retention: logs.RetentionDays.ONE_WEEK,
        removalPolicy: cdk.RemovalPolicy.DESTROY,
      }),
    })

    // Grant Bedrock permissions for KB sync
    syncKbLambda.addToRolePolicy(
      new iam.PolicyStatement({
        effect: iam.Effect.ALLOW,
    ...
    // Add /upload and /sync endpoints
    const uploadResource = api.root.addResource("upload")
    uploadResource.addMethod("POST", new apigateway.LambdaIntegration(presignedUrlLambda), {
      authorizer,
      authorizationType: apigateway.AuthorizationType.COGNITO,
    })

    const syncResource = api.root.addResource("sync")
    syncResource.addMethod("POST", new apigateway.LambdaIntegration(syncKbLambda), {
      authorizer,
      authorizationType: apigateway.AuthorizationType.COGNITO,
    })
  ...
  private createAgentCoreGateway(config: AppConfig): void {
    // Create reservation tools Lambda
    const toolLambda = new lambda.Function(this, "ReservationToolsLambda", {
      runtime: lambda.Runtime.PYTHON_3_13,
      handler: "reservation_tools_lambda.handler",
      code: lambda.Code.fromAsset(path.join(__dirname, "../../gateway/tools/reservation_tools")), // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
      timeout: cdk.Duration.seconds(30),
      environment: {
        RESERVATIONS_TABLE_NAME: this.reservationsTable.tableName,
      },
      logGroup: new logs.LogGroup(this, "ReservationToolsLambdaLogGroup", {
        logGroupName: `/aws/lambda/${config.stack_name_base}-reservation-tools`,
        retention: logs.RetentionDays.ONE_WEEK,
        removalPolicy: cdk.RemovalPolicy.DESTROY,
      }),
    })

    // Grant DynamoDB access to the reservation tools Lambda
    this.reservationsTable.grantReadWriteData(toolLambda)
    ...
    const toolSpecPath = path.join(__dirname, "../../gateway/tools/reservation_tools/tool_spec.json") // nosemgrep: javascript.lang.security.audit.path-traversal.path-join-resolve-traversal.path-join-resolve-traversal
    ...
    const gatewayTarget = new bedrockagentcore.CfnGatewayTarget(this, "GatewayTarget", {
      ...
      name: "reservation-tools-target",
      description: "Reservation tools Lambda target",
      targetConfiguration: {
        mcp: {
          lambda: {
    ...
    new cdk.CfnOutput(this, "ToolLambdaArn", {
      description: "ARN of the reservation tools Lambda",
