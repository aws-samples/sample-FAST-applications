...
def _build_session_from_records(
    span_records: List[Dict[str, Any]], eval_records: List[Dict[str, Any]]
) -> Dict[str, Session]:
    """
    ...
    for record in span_records:
        try:
            ...
            if session_id not in sessions_data:
                sessions_data[session_id] = {"spans": [], "timestamp": span.start_time}
...
def list_sessions() -> Dict[str, Any]:
    ...
    try:
        ...
        start_time = _parse_datetime(
            params.get("start_date"), end_time - timedelta(days=3)
        )
        ...
        logger.info(
            f"  start_time: {start_time} (from param: {params.get('start_date')})"
        )
        ...
        response = {
            ...
            "nextToken": None,
            "statistics": {"totalSessions": total_sessions, "averageScore": 0.0},
        }

        logger.info(f"Returning {len(response['sessions'])} sessions")
        if response["sessions"]:
            logger.info(
                f"First session timestamp: {response['sessions'][0]['timestamp']}"
            )
            logger.info(
                f"Last session timestamp: {response['sessions'][-1]['timestamp']}"
            )
...
def get_session(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if runtime_arn:
            ...
            logger.warning(
                "RUNTIME_ARN not set, will try to infer agent_id from session data"
            )

        # Use Observability SDK to get session data
        from bedrock_agentcore_starter_toolkit import Observability

        obs_client = Observability(
            agent_id=agent_id, region=os.environ.get("AWS_REGION", "us-east-1")
        )
        ...
        try:
            if agent_id:
                ...
                trace_data = obs_client.get_session_traces(session_id=session_id)
        except Exception as sdk_error:
            logger.warning(
                f"Observability SDK failed: {sdk_error}, falling back to CloudWatch client"
            )
        ...
        logger.info(
            f"Session found via SDK: {len(list(trace_data.traces.keys()))} traces, {len(trace_data.spans)} spans"
        )
        ...
        for trace_id, trace_spans in trace_data.traces.items():
            ...
            formatted_trace = {
                ...
                "startTime": datetime.fromtimestamp(
                    trace_spans[0].start_time_unix_nano / 1_000_000_000
                ).isoformat()
                if trace_spans[0].start_time_unix_nano
                else None,
                "endTime": datetime.fromtimestamp(
                    trace_spans[-1].end_time_unix_nano / 1_000_000_000
                ).isoformat()
                if trace_spans[-1].end_time_unix_nano
                else None,
        ...
        return {
            ...
            "timestamp": datetime.fromtimestamp(
                trace_data.start_time / 1_000_000_000
            ).isoformat()
    ...
    try:
        ...
        for config in configs:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to load evaluation results from config {config_id}: {e}"
                )
                continue
        ...
        if not all_evaluation_results:
            ...
            if not low_scoring_results:
                ...
                analysis_result = ai_engine.analyze_evaluation_results(
                    low_scoring_results[:limit]
                )
    ...
    try:
        ...
        if not analysis_id:
            ...
            if not low_scoring_sessions:
                raise ValueError(
                    f"No sessions found with score <= {score_threshold} to base improvements on"
                )

            logger.info(
                f"Found {len(low_scoring_sessions)} low-scoring sessions for analysis"
            )
        ...
        improvement = ai_engine.generate_prompt_improvements(
            current_prompt=current_prompt, analysis=analysis_result
        )
...
def get_analysis_status(job_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if job.get("status") == "COMPLETED" and "result" in job:
            ...
            logger.info(
                f"Returning completed job with {len(result['result'].get('patterns', []))} patterns"
            )
...
def get_prompt_improvement_status(job_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if job.get("status") == "COMPLETED" and "result" in job:
            result["result"] = convert_decimals(job["result"])
            logger.info("Returning completed prompt improvement job")
        ...
        logger.error(
            f"Error getting prompt improvement status: {str(e)}", exc_info=True
        )
...
def get_evaluation_metrics() -> Dict[str, Any]:
    ...
    try:
        ...
        end_time = _parse_datetime(params.get("end_date"), datetime.utcnow())
        start_time = _parse_datetime(
            params.get("start_date"), end_time - timedelta(days=7)
        )

        logger.info(
            f"Getting evaluation metrics for config {config_id} from {start_time} to {end_time}"
        )
...
def setup_evaluators() -> Dict[str, Any]:
    ...
    try:
        ...
        if (
            ...
            or not 0.01 <= sampling_rate <= 100.0
        ):
            return {
                "error": "samplingRate must be a number between 0.01 and 100.0"
            }, 400
        ...
        return {"message": "Successfully set up AgentCore online evaluation", **result}
...
def list_evaluation_configs() -> Dict[str, Any]:
    ...
    try:
        ...
        response = {"configurations": configs, "count": len(configs)}
...
def evaluate_on_demand() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Running on-demand evaluation for session {session_id} with evaluator {evaluator_id}"
        )
...
def evaluate_batch() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info(
            f"Running batch evaluation for {len(session_ids)} sessions with {len(evaluator_ids)} evaluators"
        )
        ...
        for session_id in session_ids:
            for evaluator_id in evaluator_ids:
                try:
                        ...
                        for span in trace.spans:
                            span_log = {
                                ...
                                "startTimeUnixNano": int(
                                    span.start_time.timestamp() * 1_000_000_000
                                ),
                                "endTimeUnixNano": int(
                    ...
                    logger.error(
                        f"Error evaluating session {session_id} with {evaluator_id}: {e}"
                    )
                    errors.append(
                        {
                            "sessionId": session_id,
...
def handler(event: dict, context: LambdaContext) -> dict:
    ...
    if isinstance(event, dict) and event.get("action") == "process_prompt_improvement":
        ...
        process_prompt_improvement_job(
            job_id, current_prompt, analysis_id, score_threshold, limit
        )
        return {}  # No response needed for async invocation
