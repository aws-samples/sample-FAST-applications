...
class AgentCoreEvaluator:
    ...
    def list_builtin_evaluators(self) -> List[Dict[str, Any]]:
        ...
        for key, evaluator_id in self.BUILTIN_EVALUATORS.items():
            ...
            evaluators.append(
                {"id": evaluator_id, "name": key, "level": level, "builtin": True}
            )
    ...
    def create_online_evaluation(
        ...
        if not self.agent_id:
            raise ValueError(
                "Agent ID not available. RUNTIME_ARN environment variable may be missing."
            )
        ...
        try:
            ...
            if (
                "ConflictException" in error_msg
                or "already exists" in error_msg.lower()
            ):
                logger.info(
                    f"Configuration {config_name} already exists, fetching existing config"
                )
                    ...
                    for config in configs:
                        if config.get("onlineEvaluationConfigName") == config_name:
                            return {
                                ...
                                "configName": config_name,
                                "evaluationArn": config.get(
                                    "onlineEvaluationConfigArn", ""
                                ),
                                "executionStatus": config.get(
    ...
    def get_evaluation_metrics(
        self, config_id: str, start_time: datetime, end_time: datetime
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for result in results:
                if "attributes" in result and isinstance(result["attributes"], dict):
                    evaluator_name = result["attributes"].get(
            ...
            for result in results:
                ...
                if "attributes" in result and isinstance(result["attributes"], dict):
                    evaluator_id = result["attributes"].get(
                        "gen_ai.evaluation.name", "unknown"
                    )
            ...
            logger.warning(
                f"Log group not found: {log_group}. No evaluations have run yet."
            )
            return {
                "totalEvaluations": 0,
                "averageScore": 0.0,
