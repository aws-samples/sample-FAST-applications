...
class CloudWatchClient:
    ...
    def __init__(self):
        ...
        if runtime_arn:
            ...
            self.runtime_log_group = f"/aws/bedrock-agentcore/runtimes/{runtime_name}-DEFAULT"
        else:
            # Fallback
    ...
    def query_sessions(
            ...
            try:
                ...
                while True:
                    ...
                    for event in events:
                        try:
                            ...
                            if timestamp < start_time or timestamp > end_time:
                                logger.debug(f"Filtering out event: timestamp={timestamp}, start={start_time}, end={end_time}")
                                continue
                    ...
                    logger.info(f"Fetching next page (processed {total_events} events so far)")
                
                logger.info(f"Found {len(sessions_dict)} sessions, {len(trace_to_session)} traces from {total_events} events")
            ...
            try:
                ...
                while True:
                    ...
                    logger.info(f"Spans query returned {len(spans_response.get('events', []))} events")
                    ...
                    for event in spans_response.get('events', []):
                        try:
                            ...
                            if not trace_id or not span_id or trace_id not in trace_to_session:
                                continue
                            ...
                            # Parse timestamps
                            start_ts = datetime.fromtimestamp(log_data.get('startTimeUnixNano', 0) / 1_000_000_000)
                            end_ts = datetime.fromtimestamp(log_data.get('endTimeUnixNano', 0) / 1_000_000_000)
                    ...
                    logger.info(f"Fetching next page of spans (processed {total_spans} events so far)")
    ...
    def _query_sessions_from_log_group(
        ...
        for event in response.get('events', []):
            ...
            try:
                ...
                if not session_id:
                    ...
                    elif 'attributes' in log_data and isinstance(log_data['attributes'], dict):
                        session_id = log_data['attributes'].get('sessionId') or log_data['attributes'].get('session.id')
        ...
        logger.info(f"Processed {events_processed} events, found {len(events_by_session)} unique sessions")
        ...
        for session_id, session_events in events_by_session.items():
            try:
                ...
                for event, log_data in session_events:
                    ...
                    if 'event_timestamp' in log_data:
                        ...
                        if isinstance(event_ts, str):
                            timestamp = datetime.fromisoformat(event_ts.replace('Z', '+00:00'))
                        else:
                ...
                if session_timestamp and (session_timestamp < start_time or session_timestamp > end_time):
                    continue
                ...
                session = Session(
                    ...
                    metadata={
                        ...
                        'source': source,
                        'trace_count': len(traces),
                        'span_count': sum(len(t['spans']) for t in traces_dict.values())
                    }
                )
    ...
    def get_session_detail(self, session_id: str) -> Optional[Session]:
        ...
        for source_name, log_group in log_sources:
            try:
                ...
                source_session = self._get_session_from_log_group(session_id, log_group, source_name)
        ...
        for event in response['events']:
            try:
                ...
                if log_data.get('sessionId') == session_id:
                    ...
                    if attrs.get('sessionId') == session_id or attrs.get('session.id') == session_id:
                        found_session = True
    ...
    def _build_session_from_events(
        ...
        for event in events:
            try:
                ...
                if start_nano and end_nano:
                    ...
                    timestamp_nano = log_data.get('timeUnixNano', event['timestamp'] * 1000000)
        ...
        session = Session(
            ...
            status=SessionStatus.COMPLETED,
            metadata={
                'log_group': self.runtime_log_group if source == 'runtime_logs' else self.SPANS_LOG_GROUP,
                'source': source
            }
        )
