...
class ImprovedCloudWatchClient:
    ...
    def __init__(self):
        ...
        if runtime_arn:
            ...
            self.runtime_logs = (
                f"/aws/vendedlogs/bedrock-agentcore/runtime/RUNTIME_LOGS/{runtime_name}"
            )
            self.otel_logs = (
    ...
    def _run_insights_query(
        ...
        if start_time < max_lookback:
            logger.info(
                f"Adjusting start_time from {start_time} to {max_lookback} (log retention limit)"
            )
            ...
            try:
                ...
                if log_group_info:
                    ...
                    if start_time < creation_time:
                        logger.info(
                            f"Adjusting start_time to log group creation time: {creation_time}"
                        )
                    ...
                    logger.info(
                        f"Log group not found (will be created on first agent invocation): {log_group}"
                    )
        ...
        if start_ms >= end_ms:
            logger.warning(
                "Time range invalid after adjustments - returning empty results"
            )
            return []
        ...
        try:
            ...
            while time.time() - start_poll < max_wait_seconds:
                ...
                if status == "Complete":
                    logger.info(
                        f"Query completed: {len(result.get('results', []))} results"
                    )
            ...
            logger.info(
                f"Log group not found: {e}. This is normal if no agent invocations have occurred yet."
            )
            return []
    ...
    def _get_spans_for_trace(
        self, trace_id: str, start_time: datetime, end_time: datetime
    ) -> List[Span]:
        """
    ...
    def query_sessions_by_score(
        ...
        for record in results:
            try:
                ...
                trace_count = int(fields.get("trace_count", 0))
                score = (
                    float(fields.get("avg_score", 0.0))
                    if fields.get("avg_score")
                    else None
                )
