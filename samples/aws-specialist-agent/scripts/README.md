...
This directory contains scripts for deploying the Fullstack AgentCore Solution Template infrastructure and frontend, plus utility scripts for skill generation, vendoring, demo user management, and diagram export.
...
These are standalone helpers, independent of the main deployment workflow above. See each file's module docstring for full details.

- `build-project-guide.py` - Generates the `fast-project-guide` skill tree, the self-describing skill that lets the deployed agent answer questions about this application. You normally do not run it by hand: `cdk synth` invokes it through the BucketDeployment bundling in `infra-cdk/lib/skills-storage-stack.ts`.
- `vendor-skills.py` - Re-creates `skills/agent-toolkit-for-aws/` from a pinned commit of the upstream `aws/agent-toolkit-for-aws` repository. See [skills/agent-toolkit-for-aws/README.md](../skills/agent-toolkit-for-aws/README.md) for the rationale.
- `create-demo-users.py` - Provisions, verifies, or deletes demo Cognito users (`create` / `verify` / `cleanup`) in a deployed stack's User Pool. Reads secrets and the target stack name from `scripts/.env` (copy `scripts/.env.example`).
- `drawio-export.sh` - Exports a draw.io (`.drawio`) diagram to an image (PNG, PDF, SVG, or JPG) via the draw.io desktop CLI. Run with `--help` for options.
- `utils.py` - Shared helpers (stack discovery from CloudFormation outputs, Cognito auth, coloured output) imported by the deployment and test scripts. Not run directly.
