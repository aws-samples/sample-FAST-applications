...
def generate_aws_exports(
    ...
    # Chat History API (Phase 3). Treated as optional so running the
    # frontend deploy against an older stack without this output does not fail;
    # the normal order (cdk deploy -> frontend) always provides it.
    if outputs.get("HistoryApiUrl"):
        aws_exports["historyApiUrl"] = outputs["HistoryApiUrl"]

    # User-selectable models for the picker. The stack emits a JSON
    # array of {key, label, available}; the physical model id stays server-side.
    # Optional for the same reason as historyApiUrl (older stacks lack it).
    if outputs.get("SelectableModelsJson"):
        aws_exports["models"] = json.loads(outputs["SelectableModelsJson"])
