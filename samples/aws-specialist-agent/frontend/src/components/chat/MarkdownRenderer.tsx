...
const components: Record<string, any> = {
  code({ className, children }: { className?: string; children?: React.ReactNode }) {
    ...
    // Fenced code block with no language (e.g. an ASCII / box-drawing diagram).
    // Inline code is always single-line, so a newline means this is a block:
    // preserve whitespace and allow horizontal scroll so box-drawing art that is
    // wider than the chat column is not wrapped into an unreadable mess.
    if (codeString.includes("\n")) {
      return (
        <div className="my-2 rounded-md overflow-hidden border border-gray-300 bg-white">
          <div className="flex items-center justify-end px-3 py-1 bg-gray-100 border-b border-gray-300">
            <CopyButton text={codeString} />
          </div>
          <pre className="m-0 overflow-x-auto p-3 text-xs leading-snug font-mono text-gray-800 whitespace-pre">
            {codeString}
          </pre>
        </div>
      )
