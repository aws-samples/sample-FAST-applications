  ...
  try {
    ...
    if (json.current_tool_use) {
      ...
      if (json.delta?.toolUse?.input === "") {
        ...
        // Pass the tool name so the consumer can synthesize a start if it never
        // saw an empty-input start event (e.g. OpenAI on Bedrock emits the full
        // input in one delta without a preceding start).
        callback({
          type: "tool_use_delta",
          toolUseId: tool.toolUseId,
