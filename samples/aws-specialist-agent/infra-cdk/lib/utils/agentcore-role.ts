			...
			inlinePolicies: {
				AgentCorePolicy: new iam.PolicyDocument({
					statements: [
						...
						// OpenAI (GPT-5.x) is served from the bedrock-mantle endpoint
						// (OpenAI Responses API), which is a separate service from
						// bedrock-runtime: it authorizes calls as bedrock-mantle:* on a
						// project resource, not bedrock:InvokeModel. Mirrors the AWS
						// managed policy AmazonBedrockMantleInferenceAccess (least
						// privilege). Reached via the in-region bedrock-mantle VPC
						// endpoint.
						new iam.PolicyStatement({
