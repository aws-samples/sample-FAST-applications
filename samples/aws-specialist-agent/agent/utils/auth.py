...
- Secure user identity extraction from JWT tokens in the AgentCore Runtime
  RequestContext (prevents impersonation via prompt injection). Both the user
  id (``sub``) and the user's Cognito group memberships (``cognito:groups``)
  are read from the same pre-validated token.
...
def _decode_validated_jwt(context: RequestContext) -> dict:
    """
    Decode the pre-validated JWT from the request context's Authorization header.

    AgentCore Runtime validates the JWT (signature, issuer, expiry) before
    passing it to the agent, so signature verification is intentionally skipped
    here. Reading identity from the token rather than the request payload
    ...
    Raises:
        ValueError: If the request headers or Authorization header are missing.
    """
...
def extract_user_id_from_context(context: RequestContext) -> str:
    ...
    The user ID is taken from the token's 'sub' claim. See
    ``_decode_validated_jwt`` for why signature verification is skipped.
    ...
    """
    claims = _decode_validated_jwt(context)
...
def extract_user_groups_from_context(context: RequestContext) -> list[str]:
    """
    Extract the user's Cognito group memberships from the JWT in the context.

    ``cognito:groups`` is present in the access token (the token the runtime
    receives), because groups are an authorization concept. Reading groups
    straight from the pre-validated token lets the V3 Pre-Token Lambda derive
    the ``department``/``role`` claims WITHOUT calling AdminListGroupsForUser —
    the membership is already proven by the token AgentCore validated.
    ...
    Returns:
        list[str]: The user's group names, or an empty list if the user is in
            no group (the Pre-Token Lambda treats this as "guest").
    """
    claims = _decode_validated_jwt(context)

    groups = claims.get("cognito:groups") or []
    # Defensive: the claim should be a JSON array, but never trust it blindly.
    if not isinstance(groups, list):
        logger.warning("cognito:groups is not a list: %r — treating as empty", groups)
        return []

    logger.info("Extracted cognito:groups from JWT: %s", groups)
