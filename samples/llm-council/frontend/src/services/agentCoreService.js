...
export const setAgentConfig = async (
  ...
  } else if (agentPattern === "llm-council-agent") {
    currentParser = await import("./councilParser.js");
  } else {
...
export const invokeAgentCore = async (
  ...
  try {
    if (!userId) {
      throw new Error(
        "No valid user ID found in session. Please ensure you are authenticated.",
      );
    }

    if (!accessToken) {
      throw new Error(
        "No valid access token found. Please ensure you are authenticated.",
      );
    }
        ...
        while (true) {
          ...
          for (const line of lines) {
            if (line.trim()) {
              if (!currentParser) {
                throw new Error(
                  "Agent parser not initialized. Ensure setAgentConfig() is called before invoking the agent.",
                );
              }
              completion = currentParser.parseStreamingChunk(
                line,
                completion,
                onStreamUpdate,
              );
