...
export class BackendStack extends cdk.NestedStack {
  ...
  private createAgentCoreRuntime(config: AppConfig): void {
    ...
    // Add LLM Council configuration if pattern is llm-council-agent
    if (pattern === "llm-council-agent" && config.backend.llm_council) {
      envVars.COUNCIL_MODELS = JSON.stringify(config.backend.llm_council.council_models)
      envVars.CHAIRMAN_MODEL = config.backend.llm_council.chairman_model
    }
